...
$current_gold_price = $gold_data['data'][0]['buy']; // Harga jual emas per gram dalam IDR
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['search'])) {
    ...
    if (empty($search_value)) {
        ...
        if ($user_result->num_rows > 0) {
            ...
            echo "<script>alert('User dengan role \'Nasabah\' tidak ditemukan.');</script>";
        }
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['convert'])) {
    ...
    if (empty($jumlah_emas) || !is_numeric($jumlah_emas)) {
        echo "<script>alert('Jumlah emas harus diisi dan berupa angka.');</script>";
    } else {
        ...
        if ($conn->query($update_query) === TRUE) {
            echo "<script>alert('Konversi berhasil! Saldo uang Anda telah diperbarui.');</script>";
        } else {
            echo "<script>alert('Terjadi kesalahan saat melakukan konversi: " . $conn->error . "');</script>";
        }
    }
}
...
<head>
    ...
    <title>Bank Sampah | Konversi Emas ke Saldo</title>
...
<body>
    ...
    function validateConversion() {
        // Cek apakah NIK sudah di-check
        const userId = document.querySelector('input[name="user_id"]');
        if (!userId) {
            alert('Harap melakukan CHECK NIK terlebih dahulu.');
