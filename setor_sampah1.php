...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['submit'])) {
    ...
    for ($i = 0; $i < count($kategori_ids); $i++) {
        ...
        if ($kategori_id && $jenis_id && $jumlah && $harga) {
            // Insert data into setor_sampah
            $insert_query = "INSERT INTO setor_sampah (id_transaksi, no, id_sampah, jumlah_kg, jumlah_rp, user_id, tanggal, waktu) 
                             VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            if ($stmt = $conn->prepare($insert_query)) {
                $stmt->bind_param("siiiiiss", $id_transaksi, $no, $jenis_id, $jumlah, $harga, $user_id, $tanggal, $waktu);
                if ($stmt->execute()) {
